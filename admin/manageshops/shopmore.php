...
<div class="w3-main" style="margin-left:300px;margin-top:60px;">
  ...
  <div class="container-md">
      <h2 style="text-align:left;"><?php echo $shopdet['sname'] ;?></h2>
      <div>
        <?php echo "<img src='../../uploads/". $shopdet['shop_img']." ' style='display: block;max-width:240px;max-height:240px;width: auto;height: auto;'>"; ?>
        </div>
        <br>
        <h3>Details</h3>
        <small><?php echo $shopdet['details']; ?></small>
        <br>
        <br>
        <h3>Location</h3>
        <small><?php echo $shopdet['addrs']; ?></small>
        <br>
        <br>
        <h3>Status</h3>
        <p class="text-success"><b><?php if($shopdet['status']==0){
          echo "Offline";
        }
        elseif($shopdet['status']==1){
              echo "Online";
        } ?></b></p>
        <ul class="nav nav-tabs">
  <li class="nav-item">
    <a class="nav-link active" href="<?php echo $_SERVER['PHP_SELF'] ; ?>">Shop Items</a>
  </li>
</ul>
...
<table class="table">
  ...
  <tbody>
    <tr>
      <?php $i=1;?>
      <?php foreach($list as $tab): ?>
      <th scope="row"><?php echo $i; ?></th>
      <td><?php echo $tab['name']; ?></td>
      <td><?php echo $tab['price']; ?></td>
      <td style="overwrap:hidden";><?php echo $tab['descp']; ?></td>
      <td><div><form action=<?php echo $_SERVER['PHP_SELF']; ?> method="POST"> 
      <input type="hidden" name="individ" value="<?php echo $tab['id']; ?>">
      <a class="btn btn-primary" name="comment" value="comment" href="viewshop.php?indiv=<?php echo $tab['id']; ?>">View Comments</a>
      </td>
      <?php  $i++; ?>
    </tr>
      <?php endforeach; ?>
